...
client.on('ready', async () => {
 ...
 client.user.setActivity(`Hydra Guard`, { type:'PLAYING' })
  
  console.log("🏳️‍🌈 #AGYP KAZANACAK!")
});
      ...
      if (msg.content == caps) {
        if (!msg.member.permissions.has("ADMINISTRATOR")) {
          if (!msg.mentions.users.first()) {
            msg.delete();
            return msg.channel.send(`${msg.member},**Bu Sunucuda CapsLock Yasak Kapatır mısınız Lütfen!**`).then(nordx => nordx.delete({timeout: 5000}))
...
client.on("message", async msg => {
 const i = await db.fetch(`${msg.guild.id}.kufur`)
    if (i) {
        const kufur = ["oç", "aq" , "yarrak" , "siktir" , "mk" , "sik" , "amk" , "göt" , "ananı" , "orospu" , "Aq"];
        if (kufur.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.permissions.has("BAN_MEMBERS")) {
                  msg.delete();
                          
                      return msg.reply('**Heey! Küfür Etme Dostum Ayıb Oluyo.**').then(nordx => nordx.delete({timeout: 5000}))
            }              
          } catch(err) {
            console.log(err);
          }
 ...
 if (veri) {
        ...
        if (reklam.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.permissions.has("BAN_MEMBERS")) {
                  msg.delete();
                    return msg.reply('**Ayb Bro! Reklam Yapma Dostum.**').then(nordx => nordx.delete({timeout: 5000}))                          
...
let hereengelle = await db.fetch(`hereengel_${msg.guild.id}`)
 if (hereengelle == 'acik') {
  ...
  if (here.some(word => msg.content.toLowerCase().includes(word)) ) {
    if (!msg.member.permissions.has("ADMINISTRATOR")) {
      msg.delete()
       return msg.reply('**Yakaladım Seni! Dostum Everyone ve Here Etiketlemek Yasak.**').then(nordx => nordx.delete({timeout: 5000}))
        }
    }
